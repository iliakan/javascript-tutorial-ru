...
- Если при вызове `open` указан только первый параметр, то для остальны параметров ииспользуются значения по умолчанию. Обычно при этом будет открыто не окно, а вкладка, что зачастую более удобно.
