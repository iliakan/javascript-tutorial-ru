...
- Если при вызове `open` указан только первый параметр, то для остальных параметров ииспользуются значения по умолчанию. Обычно при этом будет открыто не окно, а вкладка, что зачастую более удобно.
