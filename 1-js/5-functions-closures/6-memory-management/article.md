...
- ...А вот в этом случае лексическое окружение, включая переменную `value`, будет сохранено:
    ...
    */!*
    ...
    В скрытом свойстве `g.[[Scope]]` находится ссылка на объект переменных, в котором была создана `g`. Поэтому этот объект переменных останется в памяти, а в нём -- и `value`, при условии, что `value` будет использована где-то внутри. Если же внутри функции `f` переменная `value` не будет задействована, то она будет удалена.
    
    ```js
    function f() {
      var value = 123;

      function g() { return value; } // В этом случае value не удалится
