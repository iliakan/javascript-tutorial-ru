...
Ради справедливости заметим, что ошибка в точности вычислений для чисел с плавающей точкой созраняется в любом другом языке, где используется формат IEEE 754, включая Java, C, PHP, Ruby, Perl, Python. Стоит подчеркнуть -- во многих других языках есть разделение для целых чисел и чисел с плавающей точкой, поэтому проблема IEEE-754 не будет работать для таких чисел как `9999999999999999`.
