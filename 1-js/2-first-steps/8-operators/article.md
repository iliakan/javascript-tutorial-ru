...
Запятая -- единственный оператор, приоритет которого ниже присваивания. В выражении `a = (5,6)` для явного задания приоритета использованы скобки, иначе будет синтаксическая ошибка, т.к. выражение `var a = 5, 6;` приводит к `Uncaught SyntaxError: Unexpected number`.
