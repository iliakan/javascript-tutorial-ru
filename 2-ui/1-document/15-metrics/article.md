...
В примере выше нижний padding текстом НЕ заполнен. Хотя должен быть заполнен, это да. В картинке кто-то намеренно залил белым фоном пару строк.

Кроме того - не знаю как было раньше, но сейчас скролл-бар поглощается шириной правого padding'а, а не является отдельной частью.
Проверено в Chrome, Firefox, Opera.
